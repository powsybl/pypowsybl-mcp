...
from pypowsybl_mcp.tools.utils.visualisation import (
    VisualizationTools,
...
async def test_plot_substation_single_line_diagram_success(viz_tools, mock_ctx):
    ...
    with (
        patch("pypowsybl_mcp.tools.utils.visualisation.pn.SldParameters"),
        patch(
            "pypowsybl_mcp.tools.utils.visualisation.generate_download_link"
        ) as mock_gen_link,
    ):
...
async def test_plot_substation_exception_handling(viz_tools, mock_ctx):
    ...
    with patch("pypowsybl_mcp.tools.utils.visualisation.pn.SldParameters"):
        result = await viz_tools.plot_substation_single_line_diagram(
            network_id="net1", substation_id="sub1", ctx=mock_ctx
        )
